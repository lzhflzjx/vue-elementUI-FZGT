...
export default {
  ...
  methods: {
      downLoadTemp(){
        axios.get('br/api/dataBaseManage/importTemplate?id=41&type=1',{responseType:'blob'}).then(res=>{
          let url = window.URL.createObjectURL(new Blob([res.data]))
          let link = document.createElement('a')
          link.style.display = 'none'
          link.href = url
          link.setAttribute('download', '数据库批量添加模板'+new Date().getTime()+'.xls')
      ...
      upload(){
        ...
        this.loading=true;
        this.$post('/api/dataBaseManage/importFile',formData).then(res=>{
          this.loading=false;
          if(res.success){
